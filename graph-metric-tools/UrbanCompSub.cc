...
{   
    ...
    // pairs: string formatted as "s-d,s-d,..." zero-based node indices, e.g. "0-59,1-58"
    std::string pairsStr = "";
    cmd.AddValue("pairs", "List of source-sink pairs as 'src-dst,src-dst'", pairsStr);

    cmd.Parse (argc, argv);
    // parse pairsStr into fixedPairsTemp (validation after nodes are created)
    std::vector<std::pair<uint32_t,uint32_t>> fixedPairsTemp;
    if (!pairsStr.empty()) {
        std::stringstream pss(pairsStr);
        std::string tok;
        while (std::getline(pss, tok, ',')) {
            if (tok.empty()) continue;
            size_t dash = tok.find('-');
            if (dash == std::string::npos) {
                std::cerr << "Invalid pair token (expected s-d): " << tok << std::endl;
                continue;
            }
            try {
                uint32_t s = static_cast<uint32_t>(std::stoul(tok.substr(0, dash)));
                uint32_t d = static_cast<uint32_t>(std::stoul(tok.substr(dash + 1)));
                fixedPairsTemp.emplace_back(s, d);
            } catch (const std::exception &e) {
                std::cerr << "Failed to parse pair token: " << tok << " (" << e.what() << ")\n";
            }
        }
    }
    ...
    readUrbanData("scratch/UrbanCompLayout.csv", points, rectangles);
    ...
    // If pairs were passed via command line, validate them against actual node count and keep valid ones
    std::vector<std::pair<uint32_t,uint32_t>> fixedPairs;
    if (!fixedPairsTemp.empty()) {
        for (auto &pr : fixedPairsTemp) {
            if (pr.first >= nodes.GetN() || pr.second >= nodes.GetN()) {
                std::cerr << "Warning: pair (" << pr.first << "," << pr.second << ") is out of range and will be ignored\n";
                continue;
            }
            fixedPairs.push_back(pr);
        }
        if (!fixedPairs.empty()) {
            // override numSource to match provided pairs
            numSource = static_cast<int>(fixedPairs.size());
        }
    }
    ...
    std::uniform_int_distribution<> dis(4, nodes.GetN() - 1); // fallback random sink selection
    std::set<uint32_t> selectedNodes; // To track already-selected sinks when using random selection
    // If fixedPairs is not empty use those pairs otherwise use original selection logic
    if (!fixedPairs.empty()) {
        for (size_t idx = 0; idx < fixedPairs.size(); ++idx) {
            uint32_t srcIndex = fixedPairs[idx].first;
            uint32_t sinkIndex = fixedPairs[idx].second;
            Ptr<Node> node = nodes.Get(srcIndex);
            Ptr<Node> nextNode = nodes.Get(sinkIndex);
            uint16_t port = (proto == "UDP") ? 4000 : 50000;
            // (rest of per-pair setup below - identical to original, see continuation)
    
            // Update the nodeInfoMap for source
            nodeInfoMap[srcIndex].TargetID = static_cast<int32_t>(sinkIndex); // Set the target ID
            nodeInfoMap[srcIndex].TargetIP = nextNode->GetObject<Ipv4>()->GetAddress(1,0).GetLocal(); // Set the target IP
    
            // install apps and callbacks (UDP/TCP) - continue with the same code used originally
            if (proto == "UDP") {
                ...
                client.SetAttribute("Interval", TimeValue(Seconds(1 / transmissionRate)));
                ...
                if (bappcallback)
                {
                    ...
                    for (uint32_t ii = 0; ii < nextNode->GetNApplications(); ++ii)
                    {
                        udpServer = DynamicCast<UdpServer>(nextNode->GetApplication(ii));
                        if (udpServer)
                        {
                    ...
                    for (uint32_t ii = 0; ii < node->GetNApplications(); ++ii)
                    {
                        udpClient = DynamicCast<UdpClient>(node->GetApplication(ii));
                        if (udpClient)
                        {
                ...
                AddressValue remoteAddress(InetSocketAddress(nextNode->GetObject<Ipv4>()->GetAddress(1,0).GetLocal(), port));
        ...
        // original random selection behavior
        ...
        } // end for random selection
    } // end if fixedPairs else
