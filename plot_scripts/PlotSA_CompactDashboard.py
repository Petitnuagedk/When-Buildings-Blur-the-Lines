"""
PlotSA_CompactDashboard.py
--------------------------
Compact multi-panel visualisation of all results-con-SA-all-output data.

Two figures are produced and saved to plots-SA-dashboard/:

  Figure 1 – Line plots (metric vs num_nodes, one line per propagation model).
              7 metric sub-panels + 1 legend panel in a 4×2 grid.
...
Run from the repository root:
    python plot_scripts/PlotSA_CompactDashboard.py
"""
...
DATA_DIR = r"C:\Users\hugol\Documents\Firenze\B\j1\SASB-data\cone\results-con-SA-all-output"

_ap = argparse.ArgumentParser(add_help=False)
_ap.add_argument("--output", default=os.path.join(BASE_DIR, "..", "plots-SA-dashboard"))
...
def load_data(data_dir: str) -> dict:
    ...
    for model_key in MODELS:
        for n in NUM_NODES_LIST:
            path = os.path.join(data_dir, f"{model_key}_numNodes_{n}_pair_metrics.csv")
...
def plot_line_dashboard(stats: dict, out_dir: str) -> None:
    ...
    fig.suptitle("Propagation Model Comparison – All Metrics vs Node Count",
...
def plot_heatmap_comparison(stats: dict, out_dir: str) -> None:
    ...
    fig.suptitle(
        "Metric Overview by Propagation Model  (row-normalised, 0 = worst · 1 = best)",
        fontsize=15, fontweight="bold", y=1.01
    )

    for col_idx, (model_key, model_label) in enumerate(zip(model_keys, model_labels)):
        ax  = axes[col_idx]
        arr = build_mean_array(stats, model_key)        # (7, 10)
...
def plot_radar(stats: dict, out_dir: str,
    ...
    fig.suptitle("Model 'Fingerprint' Radar – mean metric values (row-normalised)",
...
if __name__ == "__main__":
    ...
    fig3 = plot_radar(stats, OUT_DIR, node_counts=[20, 50, 100])
